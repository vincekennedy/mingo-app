...
import type { DashboardGame } from '../hooks/useDashboardGames';
...
type DashboardScreenProps = {
  ...
  onLogout: () => void;
  onSelectGame: (game: DashboardGame) => void;
  onEndGame: (gameId: string) => void;
  onDuplicateSetup?: (game: DashboardGame) => void;
  onCreateGame: () => void;
  onJoinWithCode: () => void;
  onJoinPublicGame: (codeOverride?: string) => void | Promise<void>;
};
    ...
    <div className="bg-white rounded-2xl shadow-2xl p-4 sm:p-8 space-y-4 sm:space-y-6">
      ...
      {gamesLoading ? (
        ...
        <div className="space-y-3">
          {userGames.map((game) => {
            const config = game.config as WinConfigFields & {
              title?: string;
              boardSize?: number;
            } | null;
                ...
                >
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-600">
                      {game.isHost ? 'Host' : 'Player'} • {config?.boardSize}x{config?.boardSize} board
                    </p>
                    <p className="text-xs mingo-text-brand-strong mt-1">{describeWinRule(config)}</p>
                  </div>
